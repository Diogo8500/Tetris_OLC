...
void BaseEngineGL::UpdateMousePosition(int x, int y)
{
...
void BaseEngineGL::ToggleFullscreen(HWND hWnd)
{
...
void BaseEngineGL::WindowResize(void)
{
...
void BaseEngineGL::WindowUpdateScale(void)
{
...
int BaseEngineGL::SetPixelFormatGL(void)
{
...
LRESULT CALLBACK BaseEngineGL::olcWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	static BaseEngineGL* cge;


	switch (uMsg)
	{
	case WM_CREATE:
		cge = static_cast<BaseEngineGL*>(((LPCREATESTRUCT)lParam)->lpCreateParams);
...
HWND BaseEngineGL::ConstructWindow(int width, int height)
{
...
void BaseEngineGL::EnableSound()
{
	m_bEnableSound = true;
}

int BaseEngineGL::ConstructConsole(int width, int height, int fontw, int fonth)
{
...
void BaseEngineGL::Draw(int x, int y, wchar_t c, short col)
{
...
void BaseEngineGL::Fill(int x1, int y1, int x2, int y2, wchar_t c, short col)
{
	Clip(x1, y1);
	Clip(x2, y2);
	for (int x = x1; x < x2; x++)
...
void BaseEngineGL::DrawString(int x, int y, wstring c, short col)
{
...
void BaseEngineGL::DrawStringAlpha(int x, int y, wstring c, short col)
{
	for (size_t i = 0; i < c.size(); i++)
	{
		if (c[i] != L' ')
...
void BaseEngineGL::Clip(int& x, int& y)
{
	if (x < 0) x = 0;
...
void BaseEngineGL::DrawLine(int x1, int y1, int x2, int y2, wchar_t c, short col)
{
...
void BaseEngineGL::DrawCircle(int xc, int yc, int r, wchar_t c, short col)
{
	int x = 0;
	int y = r;
	int p = 3 - 2 * r;
...
void BaseEngineGL::DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3, short c, short col)
{
...
void BaseEngineGL::FillTriangle(int x1, int y1, int x2, int y2, int x3, int y3, short c, short col)
{
...
void BaseEngineGL::FillCircle(int xc, int yc, int r, wchar_t c, short col)
{
...
void BaseEngineGL::DrawSprite(int x, int y, Sprite* sprite)
{
	if (sprite == nullptr)
		return;
...
void BaseEngineGL::DrawPartialSprite(int x, int y, Sprite* sprite, int ox, int oy, int w, int h)
{
	if (sprite == nullptr)
		return;
...
void BaseEngineGL::DrawWireFrameModel(const std::vector<std::pair<float, float>>& vecModelCoordinates, float x, float y, float r, float s, short col, short c)
{
...
void BaseEngineGL::GenerateMipmapPow2(uint8_t* tex_new, uint8_t* tex_old, uint8_t* ref_alpha, int size)
{
...
void BaseEngineGL::Start()
{
	...
	// Star the thread
	thread t = thread(&BaseEngineGL::GameThread, this);
...
int BaseEngineGL::Error(const wchar_t* msg)
{
	wchar_t buff1[256];
	wchar_t buff2[256];
...
unsigned int BaseEngineGL::LoadAudioSample(std::wstring sWavFile)
{
	if (!m_bEnableSound)
		return -1;
...
void BaseEngineGL::PlaySample(int id, bool bLoop)
{
...
void BaseEngineGL::StopSample(int id)
{

}

// The audio system uses by default a specific wave format
bool BaseEngineGL::CreateAudio(unsigned int nSampleRate, unsigned int nChannels,
	unsigned int nBlocks, unsigned int nBlockSamples)
{
	...
	m_AudioThread = std::thread(&BaseEngineGL::AudioThread, this);
...
bool BaseEngineGL::DestroyAudio()
{
	m_bAudioThreadActive = false;
	return false;
}
...
void BaseEngineGL::waveOutProc(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwParam1, DWORD dwParam2)
{
...
void CALLBACK BaseEngineGL::waveOutProcWrap(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2)
{
	((BaseEngineGL*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
}
...
float BaseEngineGL::onUserSoundSample(int nChannel, float fGlobalTime, float fTimeStep)
{
	return 0.0f;
}

// Overriden by user if they want to manipulate the sound before it is played
float BaseEngineGL::onUserSoundFilter(int nChannel, float fGlobalTime, float fSample)
{
	return fSample;
}
...
float BaseEngineGL::GetMixerOutput(int nChannel, float fGlobalTime, float fTimeStep)
{
...
atomic<bool> BaseEngineGL::m_bAtomActive(false);
condition_variable BaseEngineGL::m_cvGameFinished;
mutex BaseEngineGL::m_muxGame;